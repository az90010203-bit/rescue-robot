...
import { isLatestWinsAboardBatch, isLatestWinsAboardCommand, shouldClearPendingAboardMotion } from "@adapters/pi/aBoardCommandScheduling";
...
describe("A board command scheduling", () => {
  it("treats semantic motion commands as latest-wins motion", () => {
    ...
    expect(isLatestWinsAboardCommand(command)).toBe(true);
    expect(isLatestWinsAboardBatch([
      command,
  ...
  it("keeps stop, read, and config commands on the FIFO path", () => {
    ...
    expect(isLatestWinsAboardBatch([stop])).toBe(false);
